...
def parse_run_log(file_path, model_name):
    ...
    # In gnto_msg/gnto_log, it uses "GNTO" prefix
    if model_name == "LIMAO":
        model_pattern = re.compile(r"BAO\s+(\S+)\s+([\d\.]+)")
    else:
        model_pattern = re.compile(r"GNTO\s+(\S+)\s+([\d\.]+)")
...
def analyze():
    # Paths for the 133515 archive
    base_dir = "/home/AiChaosN/Project/Phd/project/LIMAOLifeLongRLDB/gnto_ex/archive/20260206_133515/logs"
    limao_log = os.path.join(base_dir, "bao_log")
    gnto_log = os.path.join(base_dir, "gnto_log")
    ...
    print(f"--- Experiment 4: End-to-End Comparison (Archive 133515) ---")
